        ...
        .map(function(d, i) {
          var xKey = $$.getXKey(id),
            ...
            fnIsValidRibbonValue = function(val) {
              var isValidHigh =
                typeof val === 'object' &&
                val.hasOwnProperty('high') &&
                !isNaN(val.high);
              var isValidLow =
                typeof val === 'object' &&
                val.hasOwnProperty('low') &&
                !isNaN(val.low);
              return isValidHigh && isValidLow
            },
            //add value if valid and no ribbon type
            value =
              d[id] !== null && !isNaN(d[id]) && !$$.isRibbonType(id)
                ? +d[id]
                : null,
            //add y value pair if valid and ribbon type
            ribbonYvalues = !$$.isRibbonType(id)
              ? undefined
              : fnIsValidRibbonValue(d[id])
              ? d[id]
              : undefined,
            x,
            returnData;
  ...
  targets.forEach(function(t) {
    ...
    t.values.forEach(function(v) {
      if ($$.isRibbonType(v.id)) {
        ribbonHighs
          ? ys[t.id].push(v.ribbonYs.high)
          : ys[t.id].push(v.ribbonYs.low);
      } else {
...
ChartInternal.prototype.redrawGrid = function(withTransition, transition) {
  ...
  return [
    ...
    // === START PULSESHIFT CUSTOM EXTENSION ===
    (withTransition ? circles.transition(transition) : circles)
      .attr('cx', config.axis_rotated ? 0 : xv)
      .style('opacity', 1),
    (withTransition ? circlesHover.transition(transition) : circlesHover)
      .attr('cx', config.axis_rotated ? 0 : xv)
      .style('opacity', 1),
    (withTransition ? circleTexts.transition(transition) : circleTexts)
      .attr('x', xv)
      .style('opacity', 1),
    // === END PULSESHIFT CUSTOM EXTENSION ===
...
ChartInternal.prototype.generateDrawArea = function(areaIndices, isSub) {
  var $$ = this,
    ...
    value0 = function value0(d, i) {
      if ($$.isRibbonType(d)) {
        return config.data_groups.length > 0
          ? getPoints(d, i)[0][1]
          : yScaleGetter.call($$, d.id)(d.ribbonYs.low)
      } else {
        return config.data_groups.length > 0
          ? getPoints(d, i)[0][1]
    ...
    value1 = function value1(d, i) {
      if ($$.isRibbonType(d)) {
        return config.data_groups.length > 0
          ? getPoints(d, i)[1][1]
          : yScaleGetter.call($$, d.id)(d.ribbonYs.high)
      } else {
        return config.data_groups.length > 0
          ? getPoints(d, i)[1][1]
          : yScaleGetter.call($$, d.id)(d.value)
  ...
  return function(d) {
    ...
    var values =
        config.line_connectNull && !$$.isRibbonType(d)
          ? $$.filterRemoveNull(d.values)
          : d.values,
      // ===== END OPAL EXTENSION =====
      x0 = 0,
    ...
    if ($$.isAreaType(d)) {
      ...
      if ($$.isRibbonType(d)) {
        ...
        for (var i = 0; i < valuesLength; i++) {
          if (
            values[i].ribbonYs.low === null &&
            values[i].ribbonYs.high === null
          ) {
            sliceStart++;
          } else {
            break
        ...
        for (var j = valuesLength - 1; j > 0; j--) {
          if (
            values[j].ribbonYs.low === null &&
            values[j].ribbonYs.high === null
          ) {
            sliceEnd++;
          } else {
            break
...
ChartInternal.prototype.isStepType = function(d) {
  ...
  return (
    ['step', 'area-step', 'ribbon-step'].indexOf(this.config.data_types[id]) >=
    0
  )
};
ChartInternal.prototype.isSplineType = function(d) {
  var id = isString(d) ? d : d.id;
  return (
    ['spline', 'area-spline', 'ribbon-spline'].indexOf(
...
ChartInternal.prototype.isRibbonType = function(d) {
  var id = isString(d) ? d : d.id;
  return (
    ['ribbon-step', 'ribbon-spline', 'ribbon-line'].indexOf(
      this.config.data_types[id]
    ) >= 0
  )
};
