...
ChartInternal.prototype.redrawGrid = function(withTransition, transition) {
    ...
    return [
        ...
        // === START PULSESHIFT CUSTOM EXTENSION ===
        (withTransition ? circles.transition(transition) : circles).attr("cx", config.axis_rotated ? 0 : xv).style("opacity", 1),
        (withTransition ? circlesHover.transition(transition) : circlesHover).attr("cx", config.axis_rotated ? 0 : xv).style("opacity", 1),
        (withTransition ? circleTexts.transition(transition) : circleTexts).attr("x", xv).style("opacity", 1),
        // === END PULSESHIFT CUSTOM EXTENSION ===
