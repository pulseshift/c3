...
ChartInternal.prototype.isAxisNormalized = function(axisId) {
    ...
    return $$.data.targets.filter(target => $$.axis.getId(target.id) === axisId).some(target => $$.isTargetNormalized(target.id));
};
...
ChartInternal.prototype.getTotalDataSum = function() {
    ...
    const totalDataSum = flattenArray($$.data.targets.filter(target => $$.isTargetToShow(target.id)).map(target => target.values))
        .map(d => d.value)
        .reduce((p, c) => p + c, 0);
...
ChartInternal.prototype.findClosestFromTargets = function(targets, pos) {
    ...
    const candidates = targets.map(t => $$.findClosest(t.values, pos, $$.config.tooltip_horizontal ? $$.horizontalDistance.bind($$) : $$.dist.bind($$), $$.config.point_sensitivity)).filter(v => v);
